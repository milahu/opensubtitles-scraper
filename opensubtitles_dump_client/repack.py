...
class Repack:
    ...
    def __init__(self):
        ...
        # english
        self.lang_code = "eng"
        self.lang_code_short = "en"
        self.lang_code_charset_normalizer = "English"

        """
        # czech
        self.lang_code = "cze"
        self.lang_code_short = "cs"
        # no. the language-detection is often wrong
        # often it returns a similar-but-wrong language:
        # Czech, Slovak, Croatian
        self.lang_code_charset_normalizer = "Czech"
        """
        ...
        #self.index_file = f"index-grouped/index.txt.grouped.{self.lang_code}"

        self.opensubs_db_path = "opensubs.db"
        self.metadata_db_path = "opensubs-metadata.db"
        self.movie_names_db_path = "opensubs-movie-names.db"

        self.opensubs_db_first_sub_number = 1
        self.opensubs_db_last_sub_number = 9180518

        self.convert_to_utf8 = False
        self.convert_to_utf8 = True
        self.verbose_recode_content = False
        #self.verbose_recode_content = True

        # txt files are not recognized as subtitles
        # fix: rename to sub files
        # https://github.com/mpv-player/mpv/issues/4144
        self.rename_txt_to_sub = True

        self.debug_stop_after_n_groups = None
        #self.debug_stop_after_n_groups = 1
        #self.debug_stop_after_n_groups = 1000
        self.debug_stop_after_n_groups = 10

        # good for real-world users:
        # have one movie, get all subs in one language
        # bad for updates: old archives should be immutable

        self.repack_strategy = "group-by-movie-name"

        # actually this requires a first pass with group-by-movie-name
        # and in the second pass, we can group-by-movie-name-hash

        #self.repack_strategy = "group-by-movie-name-hash"
        self.repack_strategy_hash_bits = 16 # hash size in bits
        ...
        #self.repack_strategy = "group-by-sub-number"

        self.repack_strategy = "group-by-imdb-id"
        ...
        self.repack_strategy_group_size = 100
        self.repack_strategy_group_size = 10*1000
        self.repack_strategy_group_size = 10 # debug
        self.repack_strategy_group_size = 100 # xz is 40% smaller than zip
        self.repack_strategy_group_size = 1000 # xz is 40% smaller than zip

        self.debug_stop_after_n_groups = 2
        self.debug_stop_after_n_groups = 1
        self.debug_stop_after_n_groups = 100

        # /run/user/1000/opensubs-repack
        # NOTE tmpfs is limited by size of RAM
        self.main_tempdir = f"/run/user/{os.getuid()}/opensubs-repack/{self.lang_code}" # tmpfs

        self.output_dir = f"repack-grouped/{self.lang_code}/group-size-{self.repack_strategy_group_size}"
        if self.repack_strategy == "group-by-imdb-id":
            self.output_dir = f"repack-grouped/{self.lang_code}/by-imdb-id"

        self.cached_zip_dir = "done-zips"

        # global state
        #self.index_data = None
        self.opensubs_db_connection = None
        self.metadata_db_connection = None
        self.movie_names_db_connection = None

        self.opensubs_db_cursor = None
        self.metadata_db_cursor = None

        self.cached_zip_files = None

        self.keep_tempdir = False
        self.keep_tempdir = True # debug

    def is_sub_content(self, file_content):
        # example sub file:
        #   {1}{1}25.000
    ...
    def main(self):
        #global self.index_file
        #global self.index_data
        ...
        if self.repack_strategy == "group-by-movie-name":
            done_groups = 0
            sql_query = f"""
                select num, zipfile
                from subz_zipfiles
            """
            assert os.path.exists(self.movie_names_db_path), f"error: missing input file: {self.movie_names_db_path}"
            self.movie_names_db_connection = sqlite3.connect(self.movie_names_db_path)
            sql_cursor = self.movie_names_db_connection.cursor()
            ...
            for sub_num, sub_zipfile in sql_cursor.execute(sql_query):
                ...
                repacked_movie = self.repack_by_movie_name(movie_name, movie_year, self.lang_code, self.lang_code_short)
                if repacked_movie == False:
                    # dont count
                    continue
                done_groups += 1
                if self.debug_stop_after_n_groups and done_groups >= self.debug_stop_after_n_groups:
                    break
                print()

        elif self.repack_strategy == "group-by-sub-number":
            ...
            for (sub_number,) in sql_cursor.execute(sql_query, sql_args):
                ...
                if len(sub_numbers) >= self.repack_strategy_group_size:
                    print("calling repack_by_sub_numbers")
                    self.repack_by_sub_numbers(sub_numbers)
        ...
        elif self.repack_strategy == "group-by-movie-name-hash":
            #self.repack_strategy_hash_bits = 16 # hash size in bits
            raise NotImplementedError

        elif self.repack_strategy == "group-by-imdb-id":
            done_groups = 0
            # loop imdb ids of this language
            print(f"group-by-imdb-id: looping imdb ids of language {self.lang_code_short}")
            sql_query = "SELECT DISTINCT ImdbID FROM subz_metadata WHERE ISO639 = ?"
            sql_args = (self.lang_code_short,)
            sql_cursor = self.metadata_db_connection.cursor()
            for (imdb_id,) in sql_cursor.execute(sql_query, sql_args):
                ...
                if imdb_id == 0 or imdb_id > 30050816:
                    continue
                if imdb_id != 112682: continue # debug: rename duplicate != 112682: continue # debug: rename duplicate != 112682: continue # debug: rename duplicate files
                ...
                sql_cursor_2 = self.metadata_db_connection.cursor()
                ...
                sql_args = (imdb_id, self.lang_code_short)
                ...
                self.repack_by_sub_numbers(sub_numbers, group_name)
                done_groups += 1
                if self.debug_stop_after_n_groups and done_groups >= self.debug_stop_after_n_groups:
                    break
    ...
    def repack_by_sub_numbers(self, sub_numbers, group_name=None):
        sub_numbers.sort()
        if len(sub_numbers) == 0:
        ...
        group_tempdir = f"{self.main_tempdir}/{group_name}"
        #print("group_tempdir", group_tempdir)

        if os.path.exists(group_tempdir):
            content_size = self.path_getsize_recursive(group_tempdir)
            if content_size == 0:
                os.rmdir(group_tempdir)

        """
        sub_numbers_txt_path = f"{self.output_dir}/{group_name}.sub-numbers.txt"
        ...
        # so self.cached_zip_files is populated
        output_path_original_zipfiles = f"{self.output_dir}/{group_name}.original-zipfiles"
        if os.path.exists(output_path_original_zipfiles):
            ...
            for sub_number in sub_numbers:
                if not sub_number in self.cached_zip_files:
                    # FIXME missing zipfile
                    continue
                zipfile_path = self.cached_zip_files[sub_number]
        ...
        # so self.cached_zip_files is not populated
        ...
        output_path_original_zipfiles_db = f"{self.output_dir}/{group_name}.original-zipfiles.db"
        if os.path.exists(output_path_original_zipfiles_db):
            ...
            self.store_zipfiles_in_sqlite(output_path_original_zipfiles, output_path_original_zipfiles_db)
        ...
        #self.compress_group_tempdir_in_memory(group_tempdir, tar_xz_path)
        #self.compress_group_in_file(group_tempdir, tar_xz_path)

        # TODO refactor self.compress_dir_tar and self.compress_dir_squashfs
        ...
        for compressor in ["xz"]:
            ...
            #for level in [7, 8, 9]:
            for level in [9]:
                output_path = f"{self.output_dir}/{group_name}.level{level}.tar.{compressor}"
                times = self.compress_dir_tar(group_tempdir, output_path, compressor, level) or [0, 0]
        ...
        output_path = f"{self.output_dir}/{group_name}.level{level}.custom-dict.tar.{compressor}"
        times = self.compress_dir_tar(group_tempdir, output_path, compressor, level, extra_compressor_args=extra_compressor_args) or [0, 0]
        output_path_list.append([output_path, *times])
        """
        ...
        for compressor in ["xz", "gzip", "lzma", "lzo", "lz4", "zstd"]:
            output_path = f"{self.output_dir}/{group_name}.{compressor}.sqfs"
            times = self.compress_dir_squashfs(group_tempdir, output_path, compressor) or [0, 0]
            output_path_list.append([output_path, *times])
        """
        ...
        output_path = f"{self.output_dir}/{group_name}.s=on.yx=9.x=9.f=off.0=PPMd:mem2g:o32.7z"
        extra_args = ["-t7z", "-ms=on", "-myx=9", "-mx=9", "-mf=off", "-m0=PPMd:mem2g:o32"]
        times = self.compress_dir_7z(group_tempdir, output_path, extra_args) or [0, 0]
        output_path_list.append([output_path, *times])
        """
    ...
    def store_zipfiles_in_sqlite(self, output_path_original_zipfiles, output_path_original_zipfiles_db):
    ...
    def repack_by_movie_name(self, movie_name, movie_year, lang_code, lang_code_short):
        if False:
        ...
        #movie_subs = self.index_data[movie_name_year]
        # TODO MovieKind = "tv"
        # debug
        print(
        ...
        print("calling repack_by_sub_numbers")
        return self.repack_by_sub_numbers(sub_numbers, group_name)



    # https://stackoverflow.com/a/1392549/10440128
    def path_getsize_recursive(self, start_path = '.'):
        total_size = 0
    ...
    def compress_group_tempdir_in_memory(self, group_tempdir, tar_xz_path):
        """
        create tar.xz file in memory
        """
        # TODO sort files to get reproducible archive
        # https://reproducible-builds.org/docs/archives/
        #tar_xz_path = f"{self.main_tempdir}/{group_name}.py.tar.xz"
        ...
        with tarfile.open(tar_xz_path, "w:xz") as tar:
            ...
            for sub_dir in os.listdir(group_tempdir):
                ...
                tar.add(f"{group_tempdir}/{sub_dir}", sub_dir, filter=self.filter_tarinfo)
        #print(f"writing {tar_xz_path} done")

        # 453120  expected size
        # 467912  /run/user/1000/alien 3 (1992).tar.xz # self.convert_to_utf8=True
        # 467248  /run/user/1000/alien 3 (1992).tar.xz # self.convert_to_utf8=False
        # blame utf8?


    def compress_group_in_file(self, group_tempdir, tar_xz_path):
        """
    ...
    def compress_dir_tar(self, input_path, output_path, compressor="xz", level=None, extra_args=[], extra_compressor_args=[]):
    ...
    def compress_dir_squashfs(self, input_path, output_path, compressor="xz"):
        if os.path.exists(output_path):
            print(f"keeping {output_path}")
            return
            #os.unlink(output_path)
    ...
    def compress_dir_7z(self, input_path, output_path, extra_args):
    ...
    def extract_movie_sub(self, sub_number, group_tempdir):
        # TODO split this function
        # TODO recode in a separate function (self.convert_to_utf8)
        # TODO test recode
        ...
        # get the zip file
        if sub_number in self.cached_zip_files:
            zip_path = self.cached_zip_files[sub_number] # example: ../done-zips/000000123.pulp.fiction.zip
            zip_name = os.path.basename(zip_path).split(".", 1)[1] # example: pulp.fiction.zip
            zip_basename = zip_name[0:-4] # example: pulp.fiction
        else:
            if not self.opensubs_db_cursor:
                # connect only on demand
                self.opensubs_db_connection = sqlite3.connect(self.opensubs_db_path)
                # default: rows are tuples
                self.opensubs_db_connection.row_factory = sqlite3.Row # rows are dicts
                self.opensubs_db_cursor = self.opensubs_db_connection.cursor()
            #print(f"getting zipfile number {sub_number}")
            self.opensubs_db_cursor.execute("select * from subz where num = (?)", (sub_number,))
            row = self.opensubs_db_cursor.fetchone()
            if row == None:
            ...
            zip_basename = zip_name[0:-4] # example: pulp.fiction
            zip_path = f"{self.cached_zip_dir}/{sub_number:09d}.{zip_name}" # example: ../done-zips/000000123.pulp.fiction.zip
            self.cached_zip_files[sub_number] = zip_path
        ...
        sql_args = (sub_number,)
        (sub_metadata_subs_format,) = self.metadata_db_cursor.execute(sql_query, sql_args).fetchone()
        # force lowercase
        ...
        with zipfile.ZipFile(zip_path, 'r') as sub_zip:
            ...
            # sub 8257392: there seem to be multiple encodings in the metadata (filenames)
            # raw filename is 'AHÍ ESTÁ EL DETALLE.srt' which already is utf8
            # so this fails: filepath = filepath.encode("utf8").decode(metadata_encoding)
            # with metadata_encoding = "cp932"
            ...
            if not metadata_encoding in {"ascii", "utf_8", "cp437"}:
            ...
            for zip_info in sub_zip.infolist():
                ...
                filepath = zip_info.filename.strip()
                ...
                if filepath.endswith(f".({sub_number}).nfo"):
                    ...
                    # the nfo files also contain user ratings, but these are old
                    # and we should scrape the latest user ratings
                    continue
                ...
                if filepath == "":
                    ...
                    if magic_result.mime_type.startswith("text/html"):
                        ...
                        if self.is_sub_content(file_content):
                            filepath += ".sub"
                        else:
                            filepath += ".txt"
                    else:
                ...
                if self.convert_to_utf8:
                    ...
                    # guess encoding separately for each file
                    # assumption: zipfile decodes filenames with utf8 by default
                    filepath_encoding = charset_normalizer.from_bytes(filepath.encode("utf8")).best().encoding

                    #if filepath_encoding not in {"us-ascii", "utf-8", "unknown-8bit", "binary"}: # magic
                    if filepath_encoding not in {"ascii", "utf_8"}: # charset_normalizer
                        # TODO test
                        print(f"sub {group_name} {sub_number}: converting file path from {filepath_encoding} to utf8: {repr(filepath)}")
                        # TODO verify. usually type(filepath) == str
                        #filepath = filepath.decode(filepath_encoding).encode("utf8") # bytes -> str -> bytes
                        # zipfile returns all filenames as utf8 strings
                        filepath = filepath.encode("utf8").decode(filepath_encoding) # str -> bytes -> bytes
                ...
                """
                #if content_encoding != "ascii" or metadata_charset_best_match.language != self.lang_code_charset_normalizer:
                if metadata_charset_best_match.language != self.lang_code_charset_normalizer:
                ...
                file_content_string = file_content.decode(content_encoding) # bytes -> str
                ...
                if self.convert_to_utf8:
                    ...
                    if content_encoding not in {"ascii", "utf_8"}: # charset_normalizer
                        ...
                        # TODO test
                        if self.verbose_recode_content:
                        ...
                        file_content = file_content_string.encode("utf8") # str -> bytes
                        file_content_recoded_from = content_encoding

                # verify subs_format: try to parse the subtitles
                # see also: pysubs2.formats.autodetect_format
                # use some pseudo fps to avoid pysubs2.exceptions.UnknownFPSError
                try:
                    parsed_subs_format = pysubs2.SSAFile.from_string(file_content_string, fps=25).format
                except Exception as e:
                    #   File "pysubs2/substation.py", line 204, in string_to_field
                    #     return int(v)
                    # ValueError: invalid literal for int() with base 10: ''
                    print(f"sub {group_name} {sub_number}: failed to parse file {repr(filepath)}: {type(e).__name__}: {e}. using format {subs_format} from metadata")
                    parsed_subs_format = subs_format

                if parsed_subs_format == "microdvd":
                    parsed_subs_format = "sub"
                if parsed_subs_format == "ass":
                    assert subs_format in {"ass", "ssa"}, f"format mismatch. nfo/metadata: {subs_format}. parsed: {parsed_subs_format}"
                    subs_format = "ass"
                #assert parsed_subs_format == subs_format, f"format mismatch. nfo/metadata: {subs_format}. parsed: {parsed_subs_format}"
                elif parsed_subs_format != subs_format:
                    print(f"sub {group_name} {sub_number}: changing format from specified format {repr(subs_format)} to parsed format {repr(parsed_subs_format)} for file {repr(filepath)}")
                    subs_format = parsed_subs_format

                # sub_number 4353685
                # AssertionError: format mismatch. nfo/metadata: sub. parsed: srt

                # fix file extension
                filepath_base, filename_extension = os.path.splitext(filepath)

                # simple: use subs_format as file extension
                expected_extensions = set(["." + subs_format])
                if subs_format == "ssa":
                    # there are huge differences between the .SSA and .ASS file formats
                    # https://www.eztitles.com/Webhelp/EZTitles/export_subtitles_substation_alpha.htm
                    expected_extensions.add(".ass") # alias

                # remove extra ".txt" file extension
                # sub sub-4799429 4799429: adding sub file extension to file name 'space.brothers.2012.srt.txt' -> 'space.brothers.2012.srt.txt.srt'
                # blame the "hide file extension" feature of the windows file explorer
                if filename_extension not in expected_extensions:
                    expected_extensions_2 = set(map(lambda s: s + ".txt", expected_extensions))
                    if filename_extension in expected_extensions_2:
                        filepath = filepath[:-4]
                        filepath_base, filename_extension = os.path.splitext(filepath)
                ...
                if filename_extension not in expected_extensions:
                    ...
                    """
                    # keep all extensions except ".txt"
                    ...
                    print(f"sub {group_name} {sub_number}: adding sub file extension to file name -> {repr(filepath_new)}")
                    filepath = filepath_new

                filepath = f"{sub_number}." + filepath

                # limit filename length to 255 bytes
                filepath = self.cut_filename(filepath)
                ...
                # FIXME duplicate file is not renamed
                # the second file should be renamed to 39617.La_cit.2.srt
                # TODO? also rename the first file to   39617.La_cit.1.srt to fix sort order
                # $ printf "%s\n" a.z a.2.z | LC_ALL=C sort
                # a.2.z
                # a.z
                # also fixed by version-sort?
                # $ printf "%s\n" a.z a.2.z | LC_ALL=C sort --version-sort 
                # a.z
                # a.2.z
                # $ printf "%s\n" a.1.z a.2.z | LC_ALL=C sort
                # a.1.z
                # a.2.z
                """
                sub imdb-112682-len-32 39617: adding sub file extension to file name 'La_cit' -> 'La_cit.srt'
                after loop: filepath 39617.La_cit.srt
                after loop: seen_filepaths []
                after loop: duplicate_number 1
                after loop: done_rename False
                sub imdb-112682-len-32 39617: adding sub file extension to file name 'La_cit' -> 'La_cit.srt'
                after loop: filepath 39617.La_cit.srt
                after loop: seen_filepaths []
                after loop: duplicate_number 1
                ...
                old_filepath = filepath[:] # force copy # not used?
                ...
                if sub_number == 39617:
                    print("after loop: filepath", filepath)
                    print("after loop: seen_filepaths", list(seen_filepaths))
                    print("after loop: duplicate_number", duplicate_number)
                    print("after loop: done_rename", done_rename)
                ...
                filepath_abs = f"{group_tempdir}/{filepath}"
                ...
                if done_rename:
                    ...
                    for other_filepath in this_filename_filepaths:
                        if sub_number == 39617:
                            print("done_rename == True: other_filepath", other_filepath)
                        other_filepath_size = os.path.getsize(other_filepath)
                        if other_filepath_size != this_filepath_size:
                            # content is different
                            if sub_number == 39617:
                                print("done_rename == True: content is different 1")
                            continue
                        ...
                        if other_filepath_hash != this_filepath_hash:
                            # content is different
                            if sub_number == 39617:
                                print("done_rename == True: content is different 2")
                            continue
                        # content is equal -> remove this filepath
                        if sub_number == 39617:
                            print("done_rename == True: content is equal")
    ...
    def cut_filename(self, filename):
        # limit filename length to 255 bytes
