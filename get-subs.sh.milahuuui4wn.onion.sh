...
#set -x # xtrace
server_url="http://milahuuui4wn6cp37ayx46nl2k2d67tsstaq3v35kjb4xyzxcwlqwjad.onion/bin/get-subtitles"
...
[ -n "$1" ] || { echo "usage: $0 [--lang en,es,de,ru,cn] path/to/Scary.Movie.2000.720p.mp4"; exit 1; }
lang=
while (( $# > 0 )); do
case "$1" in
  --lang|-l) lang="$2"; shift 2; continue;;
  *) :;;
esac
dir="$(dirname "$1")"
...
if command -v bsdtar >/dev/null; then
  ...
  "${curl[@]}" -G --fail-with-body -D - -o - "${curl_data[@]}" "$server_url" | {
  ...
  if ! "${curl[@]}" -G --fail-with-body -o "$zip" "${curl_data[@]}" "$server_url"; then
